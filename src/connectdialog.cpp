...
void ConnectDialog::accept() {
    qDebug() << "Accept button clicked" << tabs->currentIndex();
    if(tabs->currentIndex()){
        qDebug() << "===================";
        datalogger->m_connectType = false;
        if(datalogger->testConnectTCP(urlInput->text(), 7))
        {
            qDebug() << "========accept===========";
        ...
        if( datalogger->testConnection() ) {
            qDebug() << "m_connectType = " << datalogger->m_connectType;
            return QDialog::accept();
        }
    }
