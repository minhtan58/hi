...
bool DataLogger::testConnection() {
    ...
    qDebug() << "m_connectType == " << m_connectType;
...
bool DataLogger::testConnectTCP(QString ipAdress, int port) {
    qDebug() << "Connecting..." << ipAdress << port;
    if (m_socket->state() != QTcpSocket::ConnectedState) {
        ...
        qDebug() << "Connect fail";
        showError("Error", "Connect TCP fail");
        return false;
    }
}

void DataLogger::writeTCP(const QString &s, bool end) {
    char buff[1024*5];
    write(s, APPDIR, end);
    memset(buff, 0, sizeof(buff));
    if (end) {
...
bool DataLogger::saveChannel(const QString &group, const QString &channelName, const QStringList &data) {
    ...
    m_connectType ? writeCOM(command + FRAME_DELIM + channelName + FRAME_DELIM + data.join(FRAME_DELIM))
                  : writeTCP(command + FRAME_DELIM + channelName + FRAME_DELIM + data.join(FRAME_DELIM));
#if !MOCK_TEST
    QString response = m_connectType ? readCOM() : readTCP();
#else
...
QVariantMap *DataLogger::getSensor(int idx) {
    ...
    if (idx < 12) {
        ...
        m_connectType ? writeCOM(QString("RSS") + FRAME_DELIM + channelIdx.right(2))
                      : writeTCP(QString("RSS") + FRAME_DELIM + channelIdx.right(2));
        ...
        m_connectType ? writeCOM(QString("RSS") + FRAME_DELIM + QString::number(idx + 1))
                      : writeTCP(QString("RSS") + FRAME_DELIM + QString::number(idx + 1));
#if !MOCK_TEST
        QString response = m_connectType ? readCOM() : readTCP();
#else
...
QString DataLogger::getSensorData(int idx) {
    if (idx > 17 && idx < 24) {
        ...
        m_connectType ? writeCOM(QString("RSSDATA") + FRAME_DELIM + QString::number(idx+1))
                      : writeTCP(QString("RSSDATA") + FRAME_DELIM + QString::number(idx+1));
#if !MOCK_TEST
        return m_connectType ? readCOM() : readTCP();
#else
...
#else
    //writeCOM("S7");
    m_connectType ? writeCOM("S7")
                  : writeTCP("S7");
    QStringList keys = getKeys("device");
    deviceCache.clear();
#if !MOCK_TEST
    qDebug() << "========m_connectType===========" << m_connectType;
    QString data = m_connectType ? readCOM() : readTCP();
#else
...
bool DataLogger::saveDevice(const QJsonObject &obj) {
    ...
    return true;
#else
    m_connectType ? writeCOM(QString("S8") + FRAME_DELIM +
                             obj["stationName"].toString() + FRAME_DELIM +
                             obj["stationId"].toString() + FRAME_DELIM +
                             obj["dateTime"].toString() + FRAME_DELIM +
                             obj["ipAddress"].toString() + FRAME_DELIM +
                  ...
                  : writeTCP(QString("S8") + FRAME_DELIM +
                             obj["stationName"].toString() + FRAME_DELIM +
                             obj["stationId"].toString() + FRAME_DELIM +
                             obj["dateTime"].toString() + FRAME_DELIM +
                             obj["ipAddress"].toString() + FRAME_DELIM +
...
bool DataLogger::setCellularMode(const bool state) {
    ...
    m_connectType ? writeCOM(QString("S15") + FRAME_DELIM + (state?"true":"false"))
                  : writeTCP(QString("S15") + FRAME_DELIM + (state?"true":"false"));
...
bool DataLogger::saveTransmission(const QJsonObject &obj) {
    ...
    return true;
#else
    m_connectType ? writeCOM(QString("S11") + FRAME_DELIM +
                             ...
                             obj["tcpIp"].toString())
                  : writeTCP(QString("S11") + FRAME_DELIM +
                             obj["startTime"].toString() + FRAME_DELIM +
                             obj["interval"].toString() + FRAME_DELIM +
                             obj["url"].toString() + FRAME_DELIM +
                             obj["port"].toString() + FRAME_DELIM +
...
bool DataLogger::saveChannelOrder(const QString &orderSetting) {
    ...
    m_connectType ? writeCOM(QString("S14") + FRAME_DELIM + orderSetting )
                  : writeTCP(QString("S14") + FRAME_DELIM + orderSetting );
